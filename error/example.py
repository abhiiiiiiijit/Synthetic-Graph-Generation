...
# Define the network type and distance window (250 meters)
network_type = 'drive'  # Can be 'drive', 'walk', etc.
distance = 500  # Meters from the city center

# Get the street network within 250 meters from the city center
...
for node, data in G.nodes(data=True):
    lat = data['y']
    lon = data['x']
    ax.text(lon, lat, f"({lat:.4f}, {lon:.4f})", fontsize=2, color='black')
